...
- 前端界面负责：展示 `战斗状态`、收集玩家点击形成 `PendingCommand`、显示日志和结果。
...
### 3.3 `脚本/战斗系统/bridge.ts`

职责：

- 封装当前消息楼层 `stat_data` 的读取与写回。
- 作为脚本控制器与 MVU 数据之间的统一桥接入口。

### 3.4 `脚本/战斗系统/schema_adapter.ts`
...
### 3.5 `脚本/战斗系统/engine.ts`

职责：

- 纯结算核心，不直接依赖 UI、不直接操作 DOM。
- 输入：`BattleState` + `PendingCommand` + 规则配置。
...
### 3.7 `脚本/战斗系统/commands.ts`

职责：

- 统一处理来自前端或脚本按钮的 `PendingCommand`。
- 做合法性校验与兜底修复。
- 保证敌我双方最终都走同一个命令入口。
...
### 3.12 `脚本/战斗系统/ai_bridge.ts`

职责：

- 管理“AI 能看到什么、不能看到什么”。
- 为 AI 提供可用技能、推荐目标、结算摘要，而不是完整内部公式。
- 将 AI 输出解析为统一 `PendingCommand`。
...
`脚本/战斗系统/index.ts` 当前已承担“战斗控制器 + 编排层”角色，而不是把逻辑全部塞进去。
...
const { statData } = readBattleBridgeContext(-1);

eventOn(getButtonEvent('开始战斗'), async () => {
  await runBattleTick(-1);
});

eventOn(getButtonEvent('推进战斗'), async () => {
  const command = statData.战斗状态?.待处理指令;
  await runBattleTick(-1, command ?? undefined);
});
```
...
当前 `src/wangyou/脚本/战斗系统/types.ts` 已切换到以 `Schema`
输出为中心的 DND 语义包装层，核心运行时类型已与第一版实现对齐。
...
- 第一版战斗历史当前直接写入 `stat_data.战斗归档`。

## 9. 前端协作接口

前端状态栏或战斗面板不要直接拼复杂业务逻辑。当前实现中，选择态与命令拼装已下沉到
`useBattleCommand.ts`，界面层主要消费以下能力：

- `getBattleState()`
- `getSelectableSkills(actorId)`
- `getSelectableTargets(skillId)`
- `writePendingCommand(command)` / 触发 `推进战斗`
...
当前第一版敌方自动决策已直接由 `ai.ts` 产出 `PendingCommand`，`ai_bridge.ts` 仍保留为未来文本 AI 接桥的扩展位。
...
- `select_action`：等待玩家或 AI 形成 `PendingCommand`。
...
1. `src/wangyou/schema.ts`、`src/wangyou/世界书/变量/initvar.yaml`、`src/wangyou/脚本/战斗系统/types.ts`
   已完成第一版落地。
2. `src/wangyou/脚本/战斗系统/engine.ts` 已重构为基于 `BattleState` 的流程引擎，并完成首轮模块拆分。
3. `src/wangyou/界面/状态栏` 已接入新的 `BattleState`，并具备完整第一版战斗交互闭环。
4. 当前后续重点已转为继续内容化掉落配置、视复杂度评估 `log.ts` / `schema_adapter.ts`、以及为文本 AI 桥接预留层。
...
1. 继续内容化 `规则配置.敌方掉落表`
2. 观察 `engine.ts` 日志与桥接耦合，必要时补 `log.ts` / `schema_adapter.ts`
3. 若需要 AI 文本行动桥接，再新增 `ai_bridge.ts` 与解析层

这样可以在第一版正式可用闭环的基础上，继续做结构性增强，而不是重复回到已完成的基础建设阶段。
